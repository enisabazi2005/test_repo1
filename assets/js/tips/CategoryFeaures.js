export default function loadMore() {
  let currentPage = 2;
  const load_button = document.getElementById("tips");
  ...
  const features = document.querySelector(".features_add");
  ...
  load_button.addEventListener("click", function () {
    ...
    xhr.send(`action=load_more_tips&paged=${currentPage}&step=${step}&terms=${terms}`);
  });
}