<?php
  ...
  $steps = get_field('es_tips')['steps'] ?? '';
  $category = get_field('es_tips')['es_tips_categories'] ?? '';
  $skip_posts = get_field('es_tips')['es_skip_posts'] ? 3 : 0;
  ...
  <div>
    ...
    <section class="features features_es">
      <?php
    ...
    <?php if ((int)$steps <= count($query->posts)) : ?>
      <div id="es_tips" data-typeId=<?= $steps ?> data-sec_number=<?= $category ?? 0 ?> class="load">
